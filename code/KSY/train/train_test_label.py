...
class Dataloader(pl.LightningDataModule):
    ...
    def preprocessing(self, data):
        ...
        try:
            targets = data[self.target_columns].apply(round).values.tolist()
            print(targets[:10])
        except:
            targets = []
        # 텍스트 데이터를 전처리합니다.
...
class Model(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        df = pd.DataFrame({"label":y,"prediction":logits})
        df.to_csv("./test_logits.csv")
...
if __name__ == '__main__':
    ...
    logger = WandbLogger(project="level1", group='imbalanced data', entity='moth2aflame', name='label')
    ...
    # 학습이 완료된 모델을 저장합니다.
    torch.save(model, '/data/ephemeral/home/level1/ksy/models/model_lb.pt')
