...
class WeightedMSE(torch.nn.Module):
    ...
    def forward(self, predicted, target):
        ...
        round_label = np.round(np.squeeze(target.to('cpu').detach().numpy())).astype(int).tolist()
        count_5 = round_label.count(5)
        if count_5 == 0:
            count_5 = 1
        self.weight = len(round_label)/count_5
...
class Model(pl.LightningModule):
    def __init__(self, model_name, lr):
        ...
        self.weight = [0.31,0.56, 0.87, 0.7, 0.46, 4.04]

        # 사용할 모델을 호출합니다.
        self.plm = transformers.AutoModelForSequenceClassification.from_pretrained(
            pretrained_model_name_or_path=model_name, num_labels=1)
        # Loss 계산을 위해 사용될 L1Loss를 호출합니다.
        self.loss_func = WeightedMSE(weight=torch.tensor(self.weight))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_path', default='/data/ephemeral/home/level1/data/train.csv')
    ...
    logger = WandbLogger(project="level1", group='imbalanced data', entity='moth2aflame', name='WeightedMSE')
    ...
    # 학습이 완료된 모델을 저장합니다.
    torch.save(model, '/data/ephemeral/home/level1/ksy/models/model_WMSE.pt')
