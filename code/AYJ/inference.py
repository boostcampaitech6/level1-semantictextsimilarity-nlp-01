...
class Dataloader(pl.LightningDataModule):
    ...
    def setup(self, stage='fit'):
        if stage == 'fit':
            ...
            train_data = pd.read_csv(self.train_path, index_col=0).dropna().reset_index(drop=True)
            val_data = pd.read_csv(self.dev_path)
            #all_data = pd.concat([train_data, val_data], ignore_index=True)

            # 불균형 맞춰주기 - Sampler
            train_data['round-label'] = train_data['label'].apply(lambda x: int(np.round(x)))
            class_counts = train_data['round-label'].value_counts().to_list()
            num_samples = sum(class_counts)
            labels = train_data['round-label'].to_list()
            ...
            # 학습데이터 준비
            train_inputs, train_masks, train_targets = self.preprocessing(train_data)

            # 검증데이터 준비
            val_inputs, valid_masks, val_targets = self.preprocessing(val_data)
    ...
    def train_dataloader(self):
        ...
        return torch.utils.data.DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=False, sampler=self.sampler,num_workers=8)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='rurupang/roberta-base-finetuned-sts', type=str)
    parser.add_argument('--batch_size', default=16, type=int)
    parser.add_argument('--max_epoch', default=100, type=int)
    parser.add_argument('--shuffle', default=True)
    parser.add_argument('--learning_rate', default=1e-5, type=float)
    parser.add_argument('--train_path', default='/data/ephemeral/home/codes/aug_train_row_ver2.csv')
    parser.add_argument('--dev_path', default='/data/ephemeral/home/dev.csv')
    parser.add_argument('--test_path', default='/data/ephemeral/home/dev.csv')
    parser.add_argument('--predict_path', default='/data/ephemeral/home/test.csv')
    ...
    trainer = pl.Trainer(accelerator="cpu", devices=1, max_epochs=args.max_epoch, log_every_n_steps=1)

    # Inference part
    # 저장된 모델로 예측을 진행합니다.
    model = torch.load('/data/ephemeral/home/level1/AYJ/output/model_rurupang.pt')
    ...
    output.to_csv('/data/ephemeral/home/level1/AYJ/output_rurupang.csv', index=False)