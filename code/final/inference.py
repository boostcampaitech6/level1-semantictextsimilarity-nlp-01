...
if __name__ == '__main__':
    ...
    predictions = []
    file_list = ['file_name1', 'file_name2', 'file_name3'] # 파일이름 list
    model_name = 'klue/roberta-small' # 모델 이름
    for idx in range(len(file_list)):
        model = torch.load(f'./fine-tuned/model_{file_list[idx]}.pt') # .pt 형식을 잘 맞추기
        prediction = trainer.predict(model=model, datamodule=dataloader)
        predictions.append(torch.cat(prediction))

    predictions = torch.stack(predictions)
    predictions = torch.mean(predictions, dim = 0)
    predictions = list(float(i) for i in predictions)

    # output 형식을 불러와서 예측된 결과로 바꿔주고, output.csv로 출력합니다.
    output = pd.read_csv('./data/sample_submission.csv')
    output['target'] = predictions
    output.to_csv(f'./output/output_{model_name}.csv', index=False)